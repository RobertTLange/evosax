...
def StepEllipsoidal(
    ...
    z_tilde = jnp.array(
        [
            jnp.floor(0.5 + z) * (z > 0.5)
            + (jnp.floor(0.5 + 10 * z)) * (z <= 0.5)
            for z in z_hat
        ]
    )
...
def Weierstrass(arr: chex.Array, R: chex.Array, Q: chex.Array) -> chex.Array:
    ...
    f0 = jnp.array(
        [0.5 ** k * jnp.cos(jnp.pi * 3 ** k) for k in range(k_order)]
    ).sum()
...
def SchaffersF7(arr: chex.Array, R: chex.Array, Q: chex.Array) -> chex.Array:
    ...
    def get_val(i):
        return (
            s_arr[i] ** 0.5
            + (s_arr[i] ** 0.5) * jnp.sin(50 * s_arr[i] ** 0.2) ** 2
        )
...
def GriewankRosenbrock(
    ...
    def get_val_arr(i):
        s_arr = (
            100.0 * (z_arr[i] ** 2 - z_arr[i + 1]) ** 2 + (z_arr[i] - 1) ** 2
        )
        return s_arr
...
def Gallagher101Me(arr: chex.Array, R: chex.Array, Q: chex.Array) -> chex.Array:
    ...
    for i in range(num_optima):
        w = 10 * (i == 0) + (1.1 + 8.0 * (i - 1.0) / (num_optima - 2.0)) * (
            i != 0
        )
    ...
    return oscillation_trafo(
        10.0 - max_value.squeeze()
    ) ** 2 + boundary_penalty(arr, dim)
...
def Katsuura(arr: chex.Array, R: chex.Array, Q: chex.Array) -> chex.Array:
    ...
    def get_val(i):
        ...
        for j in range(1, 33):
            s += (
                jnp.abs(2 ** j * z_vec[i] - jnp.round(2 ** j * z_vec[i]))
                / 2 ** j
            )
    ...
    return (
        (10.0 / dim ** 2) * prod - 10.0 / dim ** 2 + boundary_penalty(arr, dim)
    )
