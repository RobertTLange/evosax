...
class Differential_ES(Strategy):
    ...
    def ask(self, rng, state, params):
        ...
        y = jax.vmap(single_member_ask,
                     in_axes=(0, 0, None, None, None))(
                     rng_members, member_ids, self.num_dims,
...
def single_member_ask(rng, member_id, num_dims, archive, params):
    ...
    row_ids = jax.random.choice(rng, jnp.arange(archive.shape[0]),
                                (4,), replace=False)
    a = jax.lax.select(row_ids[0] == member_id,
                       archive[row_ids[3]], archive[row_ids[0]])
    b = jax.lax.select(row_ids[1] == member_id,
                       archive[row_ids[3]], archive[row_ids[1]])
    c = jax.lax.select(row_ids[2] == member_id,
                       archive[row_ids[3]], archive[row_ids[2]])
    ...
    y = jax.vmap(single_dimension_ask,
                 in_axes=(0, 0, None, None, None, None, None, None, None))(
                     rng_dims, dim_ids, x, a, b, c, R,
                     params["cross_over_rate"], params["diff_w"])
    return y
...
def single_dimension_ask(rng, dim_id, x, a, b, c, R, cr, diff_w):
    ...
    y_i =  (mutate_bool * a[dim_id] +  diff_w * (b[dim_id] - c[dim_id])
