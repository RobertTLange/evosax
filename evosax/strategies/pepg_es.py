...
class PEPG_ES(Strategy):
    def __init__(self,
                 ...
                 learning_rate_limit: float = 0.001):   # Stop annealing lrate
    ...
    def ask(self, rng, state, params):
        ...
        state["epsilon"] = (jax.random.normal(rng_eps, (self.batch_size, self.num_dims))
                            * state["sigma"].reshape(1, self.num_dims))
        # Note: No average baseline shenanigans as in estool version
        state["epsilon_full"] = jnp.concatenate([state["epsilon"],
                                                 -state["epsilon"]])
        epsilon = jnp.concatenate([jnp.zeros((1, self.num_dims)),
                                   state["epsilon_full"]])
    ...
    def tell(self, x, fitness, state, params):
        ...
        updates, opt_state = self.optimizer.update(change_mu,
        ...
        S = (((state["epsilon"] * state["epsilon"]
              - (state["sigma"] * state["sigma"]).reshape(1, self.num_dims))
              / state["sigma"].reshape(1, self.num_dims)))
        reward_avg = (fitness_sub[:self.batch_size] + fitness_sub[self.batch_size:]) / 2.0
        rS = reward_avg - b
        ...
        change_sigma = jnp.minimum(change_sigma, params["sigma_max_change"] * state["sigma"])
        change_sigma = jnp.maximum(change_sigma, - params["sigma_max_change"] * state["sigma"])
...
if __name__ == "__main__":
    ...
    strategy = PEPG(popsize=51, num_dims=3, learning_rate=0.02)
