...
def init_strategy(mean_init, sigma, population_size, mu):
    ...
    weights = jnp.zeros(population_size) + 1/mu
...
def tell_gaussian_strategy(x, fitness, mu, params, memory):
    ...
    mean, y_k = update_mean(sorted_solutions, mu, params, memory)
    sigma = update_sigma(y_k, mu, params, memory)
...
def update_sigma(y_k, mu, params, memory):
    """ Update stepsize sigma. """
    sigma_est = jnp.sqrt(jnp.sum((y_k[:mu].T**2 *
                                  params["weights"][:mu]), axis=1))
