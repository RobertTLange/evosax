...
def test_simple_restart():
    ...
    strategy = CMA_ES(population_size=4, solution=x)
    re_strategy = Simple_Restarter(strategy)
    restart_params = re_strategy.default_params
    restart_params = restart_params.replace(
        restart_params=restart_params.restart_params.replace(min_num_gens=2)
    )
    state = re_strategy.init(key, restart_params)

    # Run 1st ask-tell-generation
    x, state = re_strategy.ask(key, state, restart_params)
    fitness = jnp.array([1.0, 2.0, 3.0, 4.0])
    state = re_strategy.tell(x, fitness, state, restart_params)
...
def test_ipop_restart():
    ...
    num_dims = 2
    x = jnp.zeros((num_dims,))
    strategy = OpenES(population_size=4, solution=x)
...
def test_ipop_cma_es():
    ...
    num_dims = 2
    x = jnp.zeros((num_dims,))
    re_strategy = IPOP_CMA_ES(population_size=4, solution=x)
    restart_params = re_strategy.default_params
    restart_params = restart_params.replace(
        restart_params=restart_params.restart_params.replace(min_num_gens=2)
    )
...
def test_bipop_cma_es():
    ...
    num_dims = 2
    x = jnp.zeros((num_dims,))
    re_strategy = BIPOP_CMA_ES(population_size=4, solution=x)
    restart_params = re_strategy.default_params
    restart_params = restart_params.replace(
        restart_params=restart_params.restart_params.replace(min_num_gens=2)
    )
