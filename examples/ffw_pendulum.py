...
def init_policy_mlp(rng_input, sizes, scale=1e-2):
    ...
    params = {"layer_in": {"W1": W1, "b1": b1}, "layer_out": {"W2": W2, "b2": b2}}
    return params
...
def ffw_policy(params, obs):
    ...
    activations = relu_layer(params["layer_in"]["W1"], params["layer_in"]["b1"], obs)
    mean_policy = (
        jnp.dot(params["layer_out"]["W2"], activations) + params["layer_out"]["b2"]
    )
    return mean_policy
